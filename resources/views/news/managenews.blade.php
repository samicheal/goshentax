				...
				<tbody>
                    @foreach($posts as $post) 
                        <tr id="{{$post->id}}">
                            ...
                            <td><img src="{{ asset($post->featured) }}" width=40 height=40 alt="featured" ></td>
                            <td>{{$post->title}}</td>
                            <td>
                                @if($post->approved == 0)
                                    ...
                                    <div class="dropdown">
                                            ...
                                            <span class="caret"></span></button>
                                                <ul class="dropdown-menu">
                                                    ...
                                                    <li><a href="{{ route('news.edit' , $post->id) }}"><i class="fa fa-edit"></i>edit</a></li>
                                                    @if($post->approved == 0 && Auth::user()->role != "USER")
                                                        <li><a href="{{ route('news.approve' , $post->id) }}"><i class="fa fa-globe"></i>approve</a></li>
                                                    @endif
                                                    <li><a href="#" class="deleteModal" data-id="{{ $post->id }}" data-name="{{ $post->title }}"><i class="glyphicon glyphicon-trash"></i>delete</a></li>
                                                </ul>
                                    </div>
					            <!--// drop-down-for-userprofile-->
                            </td>
                 ...
                 success: function(data)
                         {
                             ...
                             //check for success
                             if(data.success){
                                toastr.info('Post successfully deleted', 'warning Alert', {timeOut: 7000}); 
                             ...
                             //check for failure
                             if(data.fail){
                                toastr.info('Could not delete post', 'warning Alert', {timeOut: 7000}); 
                                $('#deleteModal').modal('hide');  
                             }  
                         }
             });
