 ...
 <div class="grid-form">
	<div class="grid-form1">
		  <h4 id="forms-horizontal" class="text-center">Records</h4> <br/>
			<table id="manageAdvertTable" class="table">
				<thead>
					<tr>
                        <th>Id</th>
                        <th>Title</th>
				...
				<tbody>
                    @foreach($adverts as $advert) 
                        <tr id="{{$advert->id}}">
                            ...
                            <td>{{ $advert->paid }}</td>
                            <td>{{ $advert->amount - $advert->paid }}</td>
                            <td>
                                @if($advert->approved == 0)
                                    <span class="label label-danger">pending</span>
                                @else
                                    <span class="label label-success">approved</span>
                                @endif
                            </td>
                            <td>{{ $advert->created_at->toFormattedDateString() }}</td>
                                    ...
                                    <div class="dropdown">
                                            ...
                                            <span class="caret"></span></button>
                                                <ul class="dropdown-menu">
                                                    <li><a href="{{ route('dashboard') }}"><i class="fa fa-logout"></i>view</a></li>
                                                    <li><a href="{{ route('advert.edit',$advert->id) }}"><i class="fa fa-edit"></i>edit</a></li>
                                                    @if($advert->approved == 0 && Auth::user()->role != "USER")
                                                     <li><a href="{{ route('advert.approve',$advert->id) }}"><i class="fa fa-globe"></i>approve</a></li>
                                                    @endif
                                                    <li><a href="#" class="deleteModal" data-id="{{ $advert->id }}" data-name="{{ $advert->title }}"><i class="glyphicon glyphicon-trash"></i>delete</a></li>
                                                </ul>
                                    </div>
					            <!--// drop-down-for-userprofile-->
                            </td>
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header">
          ...
          <h4 class="modal-title"><i class="glyphicon glyphicon-trash"></i> Remove Advert</h4>
        </div>
...
<script>
    $(document).ready( function () {
        ...
        //load data table
        var table = $('#manageAdvertTable').DataTable();
                 ...
                 success: function(data)
                         {
                             ...
                             //check for success
                             if(data.success){
                                toastr.info('Advert successfully deleted', 'warning Alert', {timeOut: 7000}); 

                                //remove row and redraw
                                 $('#manageAdvertTable').DataTable().row('#'+data.id).remove().draw(); 
                             ...
                             //check for failure
                             if(data.fail){
                                toastr.info('Could not delete advert', 'warning Alert', {timeOut: 7000}); 
                                $('#deleteModal').modal('hide');  
                             }  
                         }
             });
...
<script>
    ...
    @if(Session::has('audit'))
        toastr.success("{{ Session::get('audit') }}");
    @endif
